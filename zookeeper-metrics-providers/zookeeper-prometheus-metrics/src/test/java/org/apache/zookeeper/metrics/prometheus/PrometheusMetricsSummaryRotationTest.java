...
public class PrometheusMetricsSummaryRotationTest {
    ...
    @Test
    public void testSummaryRotation() throws InterruptedException {
        ...
        final int timeout = summaryRotateSeconds + 1;
        for (int i = 0; i <= timeout && assertionFailed; i++) {
            SortedMap<Double, Double> quantiles = innerSummary.get().quantiles;
            try {
                assertEquals(10.0, quantiles.get(0.5), 0);
                assertEquals(10.0, quantiles.get(0.9), 0);
                assertEquals(10.0, quantiles.get(0.99), 0);
